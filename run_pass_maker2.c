...
uint16_t Network[2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)][2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)]; //ネットワーク
uint8_t Network_dis[2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)][2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)]; //距離の保存
int8_t Network_dir[2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)][2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1)]; //方向の保存
char error_mode;
DIJKSTRA Dijkstra;
...
int x_log_net[PASS_NUM]; //1f 2r 3l
int y_log_net[PASS_NUM]; //1f 2r 3l
int rc_log_net[PASS_NUM]; //1f 2r 3l
int dis_log_net[PASS_NUM]; //1f 2r 3l
int dir_log_net[PASS_NUM]; //1f 2r 3l
...
void create_DijkstraNetwork(void){
    ...
    uint16_t tmp_Number,tmp_Number2,tmp_Number3,tmp_Number4;
	//printf("%d,%d,%d,%d,%d\n",discount_v[0],discount_v[1],discount_v[2],discount_v[3],discount_v[4]);
	//printf("%d,%d,%d,%d,%d,%d\n",discount_d[0],discount_d[1],discount_d[2],discount_d[3],discount_d[4],discount_d[5]);
    ...
    pushStack_walk(&stack_xyr,tmp_Number);pushStack_walk(&stack_xyrf,tmp_Number);
    pushStack_walk(&stack_direction,8);pushStack_walk(&stack_cost,0);
    tmp_Number=trans_WalltoNetwork(1,GOAL_X ,GOAL_Y+1);
    pushStack_walk(&stack_xyr,tmp_Number);pushStack_walk(&stack_xyrf,tmp_Number);
    pushStack_walk(&stack_direction,8);pushStack_walk(&stack_cost,0);

int count=0;
	unsigned short count_number = 1;
	unsigned short Xcoordinate,Ycoordinate,Row_or_Column,Direction,dis_cost,dis_cost2;
	...
	while (1) {
		...
		//printf("cost_num %d\n",dis_cost);
		//printf("x head %d tail %d\n y head %d tail %d\n",stack_x.head,stack_x.tail,stack_y.head,stack_y.tail);
		if (dis_cost == MAX_WALKCOUNT_DIJKSTRA) {
			//printf("stack_end\n");
			break;
		}
		if(Row_or_Column==ROW){
			if(Ycoordinate <= MAZE_SQUARE_NUM-3){
				if(Direction==SLANT_NORTH){
					dis_cost2=dis_cost+1;
					VerticalCost=discount_v[dis_cost2];
				}else{VerticalCost=discount_v[0];dis_cost2=0;}
				...
				if((wall.row[Ycoordinate+1] & (1 << Xcoordinate))==0 && ((Direction==SLANT_NORTH && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_NORTH && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){
                    ...
                    if(Direction==SLANT_NORTH){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_NORTH;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_NORTH;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=VerticalCost;
                        Network[tmp_Number3][tmp_Number]=VerticalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_NORTH;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_NORTH;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
                    pushStack_walk(&stack_direction,SLANT_NORTH);
					pushStack_walk(&stack_cost,dis_cost2);
				}
			}
			if (Ycoordinate >= 1) {
				if(Direction==SLANT_SOUTH){
				...
				if((wall.row[Ycoordinate-1] & (1 << Xcoordinate))==0 && ((Direction==SLANT_SOUTH && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_SOUTH && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){
                    ...
                    if(Direction==SLANT_SOUTH){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;                        
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_SOUTH;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_SOUTH;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=VerticalCost;
                        Network[tmp_Number3][tmp_Number]=VerticalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_SOUTH;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_SOUTH;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
					pushStack_walk(&stack_direction,SLANT_SOUTH);
			...
			if (Xcoordinate <= MAZE_SQUARE_NUM-2) {
				if(Direction==SLANT_SOUTH_EAST){
					dis_cost2=dis_cost+1;
					DiagonalCost=discount_d[dis_cost2];
				}else{DiagonalCost=discount_d[0];dis_cost2=0;}
				...
				if((wall.column[Xcoordinate] & (1 << Ycoordinate))==0 && ((Direction==SLANT_SOUTH_EAST && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_SOUTH_EAST && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){                   
                    ...
                    if(Direction==SLANT_SOUTH_EAST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_SOUTH_EAST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_SOUTH_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        ...
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_SOUTH_EAST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_SOUTH_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
                    pushStack_walk(&stack_direction,SLANT_SOUTH_EAST);
				...
				if((wall.column[Xcoordinate] & (1 << (Ycoordinate+1)))==0 && ((Direction==SLANT_NORTH_EAST && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_NORTH_EAST && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){
                    ...
                    if(Direction==SLANT_NORTH_EAST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_NORTH_EAST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_NORTH_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=DiagonalCost;
                        Network[tmp_Number3][tmp_Number]=DiagonalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_NORTH_EAST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_NORTH_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
					pushStack_walk(&stack_direction,SLANT_NORTH_EAST);
			...
			if (Xcoordinate >= 1) {
				...
				if((wall.column[Xcoordinate-1] & (1 << Ycoordinate))==0 && ((Direction==SLANT_SOUTH_WEST && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_SOUTH_WEST && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){
                    ...
                    if(Direction==SLANT_SOUTH_WEST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_SOUTH_WEST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_SOUTH_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=DiagonalCost;
                        Network[tmp_Number3][tmp_Number]=DiagonalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;                        
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_SOUTH_WEST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_SOUTH_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
                    pushStack_walk(&stack_direction,SLANT_SOUTH_WEST);
                ...
                if((wall.column[Xcoordinate-1] & (1 << (Ycoordinate+1)))==0 && ((Direction==SLANT_SOUTH_WEST && Network[tmp_Number2][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA) || (Direction!=SLANT_SOUTH_WEST && Network[tmp_Number][tmp_Number3]==MAX_WALKCOUNT_DIJKSTRA))){
                    ...
                    if(Direction==SLANT_NORTH_WEST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_NORTH_WEST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_NORTH_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=DiagonalCost;
                        Network[tmp_Number3][tmp_Number]=DiagonalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_NORTH_WEST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_NORTH_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                    pushStack_walk(&stack_xyrf,tmp_Number4);
                    pushStack_walk(&stack_direction,SLANT_NORTH_WEST);
		...
		if(Row_or_Column==COLUMN){
                    ...
                    if(Direction==SLANT_EAST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_EAST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=VerticalCost;
                        Network[tmp_Number3][tmp_Number]=VerticalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_EAST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_EAST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                            pushStack_walk(&stack_xyrf,tmp_Number4);
                            pushStack_walk(&stack_direction,SLANT_EAST);
                    ...
                    if(Direction==SLANT_WEST){
                        ...
                        Network_dis[tmp_Number2][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number2]=dis_cost2;
                        Network_dir[tmp_Number2][tmp_Number3]=SLANT_WEST;
                        Network_dir[tmp_Number3][tmp_Number2]=SLANT_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column_f,Xcoordinate_f ,Ycoordinate_f);
                    }else{
                        Network[tmp_Number][tmp_Number3]=VerticalCost;
                        Network[tmp_Number3][tmp_Number]=VerticalCost;
                        Network_dis[tmp_Number][tmp_Number3]=dis_cost2;
                        Network_dis[tmp_Number3][tmp_Number]=dis_cost2;
                        Network_dir[tmp_Number][tmp_Number3]=SLANT_WEST;
                        Network_dir[tmp_Number3][tmp_Number]=SLANT_WEST;
                        tmp_Number4=trans_WalltoNetwork(Row_or_Column,Xcoordinate ,Ycoordinate);
                    }
                            pushStack_walk(&stack_xyrf,tmp_Number4);
							pushStack_walk(&stack_direction,SLANT_WEST);
...
void create_DijkstraRoute(void){
    STACK_T stack_route;
    STACK_T stack_route_f;
    uint16_t tmp_Number,tmp_Number2,tmp_Number3;
    initStack_walk(&stack_route);
    initStack_walk(&stack_route_f);
	for(int i=0;i<=MAZE_SQUARE_NUM-1;i++){
		for(int j=0;j<=MAZE_SQUARE_NUM-2;j++){
			Dijkstra.column_count[i][j]=MAX_WALKCOUNT_DIJKSTRA;
			Dijkstra.row_count[i][j]=MAX_WALKCOUNT_DIJKSTRA;
            Dijkstra.column_route[i][j]=MAX_WALKCOUNT_DIJKSTRA;
			Dijkstra.row_route[i][j]=MAX_WALKCOUNT_DIJKSTRA;
		}
	}
	Dijkstra.row_count[GOAL_X][GOAL_Y]=0;
	Dijkstra.row_count[GOAL_X+1][GOAL_Y]=0;
    ...
    tmp_Number=trans_WalltoNetwork(0,GOAL_X ,GOAL_Y);
    pushStack_walk(&stack_route,tmp_Number);
    pushStack_walk(&stack_route_f,tmp_Number);
    tmp_Number=trans_WalltoNetwork(0,GOAL_X+1 ,GOAL_Y);
    pushStack_walk(&stack_route,tmp_Number);
    pushStack_walk(&stack_route_f,tmp_Number);
    tmp_Number=trans_WalltoNetwork(1,GOAL_X ,GOAL_Y);
    ...
    int count=0;
	unsigned short count_number = 1;
	unsigned short Xcoordinate,Ycoordinate,Row_or_Column,cost;
    unsigned short Xcoordinate_f,Ycoordinate_f,Row_or_Column_f,cost_f;  
    unsigned short Xcoordinate2,Ycoordinate2,Row_or_Column2,cost2;  
	while (1) {

		tmp_Number = popStack_walk(&stack_route);
        trans_NetworktoWall(&Row_or_Column, &Xcoordinate, &Ycoordinate, tmp_Number);
        tmp_Number2 = popStack_walk(&stack_route_f);
        trans_NetworktoWall(&Row_or_Column_f, &Xcoordinate_f, &Ycoordinate_f, tmp_Number2);
        cost=Dijkstra.row_count[Xcoordinate][Ycoordinate];
        if(Row_or_Column==ROW){
            cost=Dijkstra.row_count[Xcoordinate][Ycoordinate];
         }
         else{
            cost=Dijkstra.column_count[Ycoordinate][Xcoordinate];
         }        
        ...
        for(int i=0;i<2*MAZE_SQUARE_NUM*(MAZE_SQUARE_NUM-1);i++){
            if(Network[tmp_Number][i]!=MAX_WALKCOUNT_DIJKSTRA){
                trans_NetworktoWall(&Row_or_Column2, &Xcoordinate2, &Ycoordinate2, i);
                if(Row_or_Column2==ROW){
                    cost2=Dijkstra.row_count[Xcoordinate2][Ycoordinate2];
                }
                else{
                    cost2=Dijkstra.column_count[Ycoordinate2][Xcoordinate2];
                }
                if(cost2>cost+Network[tmp_Number][i]){
                    if(Row_or_Column2==ROW){
                        Dijkstra.row_count[Xcoordinate2][Ycoordinate2]=cost+Network[tmp_Number][i];
                        Dijkstra.row_route[Xcoordinate2][Ycoordinate2]=tmp_Number;
                    }
                    else{
                        Dijkstra.column_count[Ycoordinate2][Xcoordinate2]=cost+Network[tmp_Number][i];
                        Dijkstra.column_route[Ycoordinate2][Xcoordinate2]=tmp_Number;
                    }
                    pushStack_walk(&stack_route,i);
                    pushStack_walk(&stack_route_f,tmp_Number);
                }
            }
...
void pass_maker_DijkstraNetwork(void){
    ...
    char turn_num=0;
    uint32_t count=0;
    unsigned short Xcoordinate,Ycoordinate,Row_or_Column,dista;
    int8_t direc,direc0,delta_direc;
	pass_count = 0;
    create_DijkstraNetwork();
    create_DijkstraRoute();
	maze_display_Dijkstra();
    ...
    rc_log_net[count]=0;
    dis_log_net[count]=0;
    dir_log_net[count]=SLANT_SOUTH;
    count++;

    pass[pass_count]=1;
    pass_count++;

        //trans_WalltoNetwork(0,GOAL_X ,GOAL_Y);
    
while(1){
        if(Row_or_Column==0){
        tmp_Number2=Dijkstra.row_route[Xcoordinate][Ycoordinate];
        trans_NetworktoWall(&Row_or_Column, &Xcoordinate, &Ycoordinate, tmp_Number2);
        }else{
        tmp_Number2=Dijkstra.column_route[Ycoordinate][Xcoordinate];
        trans_NetworktoWall(&Row_or_Column, &Xcoordinate, &Ycoordinate, tmp_Number2);
        }
        dista=Network_dis[tmp_Number][tmp_Number2]+1;
        direc=Network_dir[tmp_Number][tmp_Number2];
	x_log_net[count]=Xcoordinate;
    y_log_net[count]=Ycoordinate;
    rc_log_net[count]=Row_or_Column;
    dis_log_net[count]=dista;
    dir_log_net[count]=direc;
   delta_direc=direc-direc0;
    if(delta_direc > 5){delta_direc-=8;}else if(delta_direc < -5){delta_direc+=8;}
    count++;

    if(delta_direc==1){
        pass[pass_count]= -100;//右45その場旋回
    }else if(delta_direc== -1){
        pass[pass_count]= -101;//左45その場旋回
    }else if(delta_direc== 2){
        pass[pass_count]= -102;//右90その場旋回
    }else if(delta_direc== -2){
        pass[pass_count]= -103;//左90その場旋回
    }else{
        pass[pass_count]= -1;
    }
    ...
    //pass[pass_count]= -2;
    pass_count++;
    if(direc==SLANT_NORTH || direc==SLANT_SOUTH || direc==SLANT_EAST || direc==SLANT_WEST){
    pass[pass_count]=dista*2;
    }else{
    pass[pass_count]=SLANT_PASS_COUNT+dista;
    }
    pass_count++;
    if((Xcoordinate == GOAL_X || Xcoordinate == GOAL_X+1) && (Ycoordinate == GOAL_Y || Ycoordinate == GOAL_Y+1)){break;}

    tmp_Number=tmp_Number2;
    direc0=direc;
    ...
    pass_count = 1;
while (1) {		//パス圧縮    
    if (pass[pass_count] == 0) {break;}

    if(pass[pass_count] == -100 && pass[pass_count-1] >0 && pass[pass_count-1] < SLANT_PASS_COUNT){
        if(pass[pass_count-1]==1){pass[pass_count-1]=-1;}else{pass[pass_count-1]--;}
        turn_num=0;
    while(1){
        if(pass[pass_count]==-100){turn_num++;}
        if(pass[pass_count]==-102){turn_num+=2;}
        if(pass[pass_count]>1 && pass[pass_count]<SLANT_PASS_COUNT){break;} 
        if(pass[pass_count]>SLANT_PASS_COUNT+1){break;}
        pass[pass_count]=-1;
        pass_count++;
    }
        if(turn_num==1){pass[pass_count-1]=-8;}
        if(turn_num==2){pass[pass_count-1]=-4;}
        if(turn_num==3){pass[pass_count-1]=-10;}
        if(turn_num==4){pass[pass_count-1]=-6;}  
        if(pass[pass_count]==1){pass[pass_count]=-1;}else{pass[pass_count]--;}
    }

    if(pass[pass_count] == -101 && pass[pass_count-1] >0 && pass[pass_count-1] < SLANT_PASS_COUNT){
        if(pass[pass_count-1]==1){pass[pass_count-1]=-1;}else{pass[pass_count-1]--;}
        turn_num=0;
    while(1){
        if(pass[pass_count]==-101){turn_num++;}
        if(pass[pass_count]==-103){turn_num+=2;}
        if(pass[pass_count]>1 && pass[pass_count]<SLANT_PASS_COUNT){break;} 
        if(pass[pass_count]>SLANT_PASS_COUNT+1){break;}
        pass[pass_count]=-1;
        pass_count++;
    }
        if(turn_num==1){pass[pass_count-1]=-9;}
        if(turn_num==2){pass[pass_count-1]=-5;}
        if(turn_num==3){pass[pass_count-1]=-11;}
        if(turn_num==4){pass[pass_count-1]=-7;}  
        if(pass[pass_count]==1){pass[pass_count]=-1;}else{pass[pass_count]--;}
    }



    if(pass[pass_count] == -100 && pass[pass_count-1] > SLANT_PASS_COUNT){
        if(pass[pass_count-1]==SLANT_PASS_COUNT+1){pass[pass_count-1]=-1;}else{pass[pass_count-1]--;}
        turn_num=0;
    while(1){
        if(pass[pass_count]==-100){turn_num++;}
        if(pass[pass_count]==-102){turn_num+=2;}
        ...
        if(turn_num==1){pass[pass_count-1]=-12;}
        if(turn_num==2){pass[pass_count-1]=-16;}
        if(turn_num==3){pass[pass_count-1]=-14;}  
        if(pass[pass_count]==SLANT_PASS_COUNT+1){pass[pass_count]=-1;}else{pass[pass_count]--;}
    }

    if(pass[pass_count] == -101 && pass[pass_count-1] > SLANT_PASS_COUNT){
        if(pass[pass_count-1]==SLANT_PASS_COUNT+1){pass[pass_count-1]=-1;}else{pass[pass_count-1]--;}
        turn_num=0;
    while(1){
        if(pass[pass_count]==-101){turn_num++;}
        ...
        if(turn_num==1){pass[pass_count-1]=-13;}
        if(turn_num==2){pass[pass_count-1]=-17;}
        if(turn_num==3){pass[pass_count-1]=-15;}  
        if(pass[pass_count]==SLANT_PASS_COUNT+1){pass[pass_count]=-1;}else{pass[pass_count]--;}
    }
                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  /* call the computational subroutine */
    ...
    run_shortest(3000,10000,0,0,0,0,speed300_exploration,0.99,1);
  ...
  memcpy(start_of_pr,Network,bytes_to_copy); 
   ...
   /* create a 2-by-2 array of unsigned 16-bit integers */
  plhs[6] = mxCreateNumericArray(2,dims0,mxINT32_CLASS,mxREAL);
  start_of_pr = (unsigned char *)mxGetData(plhs[6]);
  bytes_to_copy = PASS_NUM * mxGetElementSize(plhs[6]);
  memcpy(start_of_pr,x_log_net,bytes_to_copy);

  /* create a 2-by-2 array of unsigned 16-bit integers */
  plhs[7] = mxCreateNumericArray(2,dims0,mxINT32_CLASS,mxREAL);
  start_of_pr = (unsigned char *)mxGetData(plhs[7]);
  bytes_to_copy = PASS_NUM * mxGetElementSize(plhs[7]);
  memcpy(start_of_pr,y_log_net,bytes_to_copy);   

       /* create a 2-by-2 array of unsigned 16-bit integers */
  plhs[8] = mxCreateNumericArray(2,dims0,mxINT32_CLASS,mxREAL);
  start_of_pr = (unsigned char *)mxGetData(plhs[8]);
  bytes_to_copy = PASS_NUM * mxGetElementSize(plhs[8]);
  memcpy(start_of_pr,rc_log_net,bytes_to_copy);

       /* create a 2-by-2 array of unsigned 16-bit integers */
  plhs[9] = mxCreateNumericArray(2,dims0,mxINT32_CLASS,mxREAL);
  start_of_pr = (unsigned char *)mxGetData(plhs[9]);
  bytes_to_copy = PASS_NUM * mxGetElementSize(plhs[9]);
  memcpy(start_of_pr,dis_log_net,bytes_to_copy);    

       /* create a 2-by-2 array of unsigned 16-bit integers */
  plhs[10] = mxCreateNumericArray(2,dims0,mxINT32_CLASS,mxREAL);
  start_of_pr = (unsigned char *)mxGetData(plhs[10]);
  bytes_to_copy = PASS_NUM * mxGetElementSize(plhs[10]);
  memcpy(start_of_pr,dir_log_net,bytes_to_copy);  
}
